# Copyright © 2026 Oracle and/or its affiliates.
#
# This software is under the Apache License 2.0
# (LICENSE-APACHE or http://www.apache.org/licenses/LICENSE-2.0) or Universal Permissive License
# (UPL) 1.0 (LICENSE-UPL or https://oss.oracle.com/licenses/upl), at your option.

import pytest
import yaml

from pyagentspec import Agent
from pyagentspec.llms import OllamaConfig
from pyagentspec.property import StringProperty
from pyagentspec.serialization import AgentSpecSerializer
from pyagentspec.tools import ServerTool

# We only perform simple tests to ensure that loader and exporter work as expected.
# Extensive tests for these classes are already implemented in wayflowcore.


@pytest.fixture
def mock_tool_fixture():

    from wayflowcore.tools import tool

    @tool(description_mode="only_docstring")
    def mock_tool() -> str:
        """Mocked tool"""
        return "WayFlow is the Agent Spec reference framework."

    return mock_tool


def test_wayflow_agent_can_be_converted_to_agentspec(mock_tool_fixture) -> None:
    from wayflowcore.agent import Agent
    from wayflowcore.models import OpenAICompatibleModel

    from pyagentspec.adapters.wayflow import AgentSpecExporter

    llm = OpenAICompatibleModel(
        model_id="agi_model",
        base_url=f"http://url_to_my_agi_model/v1",
    )
    agent = Agent(
        name="wayflow_assistant",
        llm=llm,
        custom_instruction="Use tools to solve tasks.",
        tools=[mock_tool_fixture],
        initial_message=None,
    )

    exporter = AgentSpecExporter()
    agentspec_yaml = exporter.to_yaml(agent)
    agentspec_dict = yaml.safe_load(agentspec_yaml)
    assert "component_type" in agentspec_dict
    assert agentspec_dict["component_type"] == "Agent"
    assert agentspec_dict["name"] == "wayflow_assistant"
    # We cannot retrieve the system prompt properly yet
    assert agentspec_dict["system_prompt"] == "Use tools to solve tasks."
    # Check LLM
    assert "llm_config" in agentspec_dict
    assert "component_type" in agentspec_dict["llm_config"]
    assert agentspec_dict["llm_config"]["component_type"] == "OpenAiCompatibleConfig"
    # Check Tools
    assert "tools" in agentspec_dict
    assert isinstance(agentspec_dict["tools"], list)
    assert len(agentspec_dict["tools"]) == 1
    assert "component_type" in agentspec_dict["tools"][0]
    assert agentspec_dict["tools"][0]["component_type"] == "ServerTool"
    assert agentspec_dict["tools"][0]["name"] == "mock_tool"


def test_agentspec_agent_can_be_converted_to_wayflow(mock_tool_fixture) -> None:

    from wayflowcore.agent import Agent as WayFlowAgent
    from wayflowcore.models import OllamaModel

    from pyagentspec.adapters.wayflow import AgentSpecLoader

    agent = Agent(
        name="wayflow_assistant",
        description="You are a helpful assistant",
        llm_config=OllamaConfig(
            name="agi_model",
            model_id="agi_model",
            url="url_to_my_agi_model",
        ),
        tools=[
            ServerTool(name="mock_tool", inputs=[], outputs=[StringProperty(title="mock_tool")])
        ],
        system_prompt="Use tools to solve tasks.",
    )
    agentspec_yaml = AgentSpecSerializer().to_yaml(agent)

    wayflow_assistant = AgentSpecLoader(tool_registry={"mock_tool": mock_tool_fixture}).load_yaml(
        agentspec_yaml
    )
    assert isinstance(wayflow_assistant, WayFlowAgent)
    assert wayflow_assistant.name == "wayflow_assistant"
    assert isinstance(wayflow_assistant.llm, OllamaModel)
    assert wayflow_assistant.description == "You are a helpful assistant"
    assert len(wayflow_assistant.tools) == 1
