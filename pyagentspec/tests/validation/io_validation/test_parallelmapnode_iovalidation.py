...
def test_parallel_map_node_infers_inputs_as_lists_of_inner_flow_inputs(
    double_number_flow: Flow,
) -> None:
    parallel_map_node = ParallelMapNode(name="node", subflow=double_number_flow)
    assert parallel_map_node.inputs == [
        Property(
            json_schema={
    ...
    assert parallel_map_node.outputs == [
        Property(
            json_schema={
...
def test_parallel_map_node_infers_output_aligns_with_reduction_method(
    double_number_flow: Flow,
) -> None:
