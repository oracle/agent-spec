...
YAML_FILE_NAME = "agent_with_1_tool_multiple_outputs.yaml"
...
# Track if multiplication_summation_tool was called
multiplication_summation_tool_called = False


def multiplication_summation_tool(a, b):
    global multiplication_summation_tool_called
    multiplication_summation_tool_called = True
...
def agentspec_component(
    ...
    tool_registry = {"multiplication_summation_tool": multiplication_summation_tool}
    component = load_agentspec_config(agentspec_configuration, tool_registry=tool_registry)
    return component


def test_valid_configuration_agent_with_1_tool_multiple_outputs_can_be_loaded(
    agentspec_component,
) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ("3, 6", {"product": 18, "sum": 9}),
        ("3, 3", {"product": 9, "sum": 6}),
    ],
)
def test_agent_with_1_tool_multiple_outputs_can_be_executed(
    agentspec_component, user_message, expected_llm_output, local_deterministic_llm_server
) -> None:
    global multiplication_summation_tool_called
    multiplication_summation_tool_called = False  # Reset before test
    ...
    # Assert multiplication_summation_tool was called
    assert multiplication_summation_tool_called is True, "The tool was not called as expected."

    if hasattr(result, "outputs"):
        assert (
            str(expected_llm_output["product"]) in result.outputs["llm_output"]
            and str(expected_llm_output["sum"]) in result.outputs["llm_output"]
        ), f"Expected {expected_llm_output} for {user_message}"
    elif hasattr(result, "agent_messages"):
        agent_messages = result.agent_messages[-1]
        assert (
            str(expected_llm_output["product"]) in agent_messages
            and str(expected_llm_output["sum"]) in agent_messages
        ), f"Expected {expected_llm_output} for {user_message}"
    else:
